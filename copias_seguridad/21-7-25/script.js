...
function renderDashboardJugador(metricasPorJugador) {
  ...
  if (!select || !resumenDiv || !graficosDiv || !tablaAnualDiv) return;
  // Poblar selector
  const jugadores = Object.keys(metricasPorJugador);
  ...
  function mostrarDashboard() {
    ...
    if (!m) return;
    // --- TABLA ANUAL DE ESTADÍSTICAS ---
    // Agrupar por año: puntos, goles, asistencias
    const dataRowsAnual = window.dataRowsOriginal;
    ...
    dataRowsAnual.forEach(cols => {
      if (cols[idxJugadorAnual] !== jugador) return;
      const fecha = cols[idxFechaAnual];
      let year = '';
      if (fecha.includes('-')) year = fecha.split('-')[0];
      else if (fecha.includes('/')) year = fecha.split('/').pop();
      else year = fecha.substring(0, 4);
      if (!statsPorAnio[year]) statsPorAnio[year] = { puntos: 0, goles: 0, partidos: 0 };
      statsPorAnio[year].puntos += Number(cols[idxPuntosAnual]) || 0;
      statsPorAnio[year].goles += Number(cols[idxGolesAnual]) || 0;
      statsPorAnio[year].partidos += 1;
    });
    // Renderizar tabla
    ...
    anios.forEach(anio => {
      ...
      tablaHtml += `<tr><td style="padding:0.6em 1em;text-align:center;">${anio}</td><td style="padding:0.6em 1em;text-align:center;">${s.puntos}</td><td style="padding:0.6em 1em;text-align:center;">${s.goles}</td><td style="padding:0.6em 1em;text-align:center;">${s.partidos}</td></tr>`;
    });
    ...
    // Estructura moderna tipo panel lateral + panel principal
    resumenDiv.innerHTML = `
      <div class="dashboard-jugador-layout" style="display:flex;flex-wrap:wrap;gap:2.5em;justify-content:center;align-items:stretch;">
        <div class="dashboard-jugador-panel-izq" style="background:#23263a;border-radius:18px;box-shadow:0 2px 16px #23263a44;padding:2em 1.5em;min-width:220px;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;gap:1.2em;">
          ...
          <h2 style="color:#ffd700;text-align:center;margin-bottom:0.2em;font-size:1.5em;">${jugador}</h2>
          <div style="color:#ffd700;font-size:1.15em;font-weight:600;">${m.puntos} pts</div>
          <div style="display:flex;flex-direction:column;align-items:center;gap:0.5em;margin-top:0.5em;">
            <div style="color:#ffd700;">Partidos: <b>${m.partidos}</b></div>
            <div style="color:#ffd700;">Goles: <b>${m.goles}</b></div>
            <div style="color:#ffd700;">Dif. Gol: <b>${m.difGol}</b></div>
          </div>
        ...
        <div class="dashboard-jugador-panel-der" style="flex:1;min-width:320px;display:flex;flex-direction:column;gap:2em;">
          <div class="dashboard-jugador-graficos-row" style="display:flex;flex-wrap:wrap;gap:2em;justify-content:center;align-items:flex-start;">
            <div>
    ...
    `;
    // Gráfico de barras con barras máximas resaltadas
    if (goles.length > 0) {
      const ctxBar = document.getElementById('graficoGolesJugador').getContext('2d');
      const max = Math.max(...goles);
      const bgColors = goles.map(v => v === max ? '#4fc3f7' : '#ffd70099');
      const borderColors = goles.map(v => v === max ? '#1976d2' : '#ffd700');
      new Chart(ctxBar, {
        type: 'bar',
        data: {
          labels: fechas,
