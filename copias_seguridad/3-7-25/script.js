...
function renderHistorialPartidos(dataRows, idxFecha, idxJugador, idxGoles, idxPuntos) {
  ...
  function mostrarDetalle(fecha) {
    ...
    // --- CARD ---
    let html = `<div class="partidos-cards-container">`;
    html += `<div class="partido-card" data-show="true">`;
    html += `<div class="partido-card-header">
      <span class="partido-card-fecha">${fecha}</span>
      <span class="partido-card-resultado">`;
    if (ganadores.length && perdedores.length) {
      html += `${golesGanadores} - ${golesPerdedores}`;
    } else if (empatados.length) {
      html += `${golesEmpatados} - ${golesEmpatados}`;
    } else {
      html += `-`;
    }
    html += `</span></div>`;
    ...
    if (ganadores.length && perdedores.length) {
      html += `<span class="partido-card-equipo ganador">GANADOR`;
      html += `<span class="partido-card-badge">${golesGanadores} Goles</span></span>`;
      html += `<span style="font-size:1.3em;color:#ffd700;">vs</span>`;
      html += `<span class="partido-card-equipo">PERDEDOR<span class="partido-card-badge" style="background:#d32f2f;color:#fff;">${golesPerdedores} Goles</span></span>`;
    } else if (empatados.length) {
      html += `<span class="partido-card-equipo" style="background:linear-gradient(90deg,#b3e5fc 0%,#ffd700 100%);color:#23263a;">EQUIPO 1<span class="partido-card-badge">${golesEmpatados} Goles</span></span>`;
      html += `<span style="font-size:1.3em;color:#ffd700;">vs</span>`;
      html += `<span class="partido-card-equipo" style="background:linear-gradient(90deg,#b3e5fc 0%,#ffd700 100%);color:#23263a;">EQUIPO 2<span class="partido-card-badge">${golesEmpatados} Goles</span></span>`;
    }
    html += `</div>`;
    html += `<div class="partido-card-divider"></div>`;
    ...
    if (ganadores.length && perdedores.length) {
      // Ganadores
      html += `<div class="equipo-columna equipo-ganador">`;
      ganadores.forEach(cols => {
        ...
        let imgSrc = `img/jugadores/jugador-${normalizarNombre(nombre)}.png`;
        html += `<span class=\"partido-card-jugador\">`;
        // Primero los goles
        if (goles > 0) html += `<span class=\"partido-card-goles\">${'<span class=\\"icono-gol\\">⚽</span>'.repeat(goles)}</span>`;
        // Luego el nombre
        html += `<span class=\"nombre-jugador\">${nombre}</span>`;
        // Por último la foto
        html += `<img src=\"${imgSrc}\" onerror=\"this.onerror=null;this.src='img/jugadores/jugador-vacio.png'\" alt=\"${nombre}\">`;
        html += `</span>`;
      });
      html += `</div>`;
      // Separador cancha
      ...
      // Perdedores
      html += `<div class=\"equipo-columna equipo-perdedor\">`;
      perdedores.forEach(cols => {
        const nombre = cols[idxJugador];
        const goles = Number(cols[idxGoles]);
        function normalizarNombre(nombre) {
        ...
        let imgSrc = `img/jugadores/jugador-${normalizarNombre(nombre)}.png`;
        html += `<span class="partido-card-jugador"><img src="${imgSrc}" onerror="this.onerror=null;this.src='img/jugadores/jugador-vacio.png'" alt="${nombre}"><span class="nombre-jugador">${nombre}</span>`;
        if (goles > 0) html += `<span class="partido-card-goles">${'<span class\\"icono-gol\\">⚽</span>'.repeat(goles)}</span>`;
        html += `</span>`;
      });
      html += `</div>`;
      ...
      html += `<div style="flex:1;min-width:140px;background:linear-gradient(120deg,#b3e5fc22 0%,#ffd70022 100%);border-radius:12px;padding:0.7em 0.5em;margin-bottom:0.5em;box-shadow:0 2px 8px #ffd70020;">`;
      empatados.forEach(cols => {
        const nombre = cols[idxJugador];
        const goles = Number(cols[idxGoles]);
        let imgSrc = `img/jugadores/${normalizarNombreArchivo(nombre)}.png`;
        html += `<span class="partido-card-jugador"><img src="${imgSrc}" onerror="this.onerror=null;this.src='img/jugadores/jugador-vacio.png'" alt="${nombre}"><span class="nombre-jugador">${nombre}</span>`;
        if (goles > 0) html += `<span class="partido-card-goles">${'<span class=\"icono-gol\">⚽</span>'.repeat(goles)}</span>`;
        html += `</span>`;
      });
