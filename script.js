...
// Helper: escapa texto para atributos HTML
...
function renderRegistroLogros() {
  ...
  let html = '';
    // Colores por medalla y estructura de HTML
  const coloresMedalla = {
    ...
    config.forEach(medalla => { // Iterar sobre cada medalla
...
window.renderRecords = function(year, mode) {
  ...
  function avatarFor(jugador) {
    ...
    return `<img class="avatar-img" src="${src}" onerror="this.src='img/jugadores/jugador-vacio.png'" alt="${jugador}">`;
  }
  ...
  // Puntos Totales (histórico) - nueva métrica para el record solicitado
  const puntosTotales = Object.entries(metricasHistoric).map(([j,m])=>({jugador:j,value: (m.puntos||0), partidos:m.partidos||0})).sort((a,b)=> b.value - a.value || b.partidos - a.partidos);
  ...
  // Nueva: calcular todas las rachas de partidos con al menos N goles por jugador
  function calcularRachasGoles(dataRows, minGoles = 1) {
    const partidosPorJugador = {};
    dataRows.forEach((cols, idxRow) => {
      const j = cols[idxJugador];
      const fecha = cols[window.idxFecha];
      const golesRaw = (typeof window.idxGoles === 'number' && window.idxGoles >= 0) ? Number(cols[window.idxGoles]) : 0;
      if (golesRaw === null || typeof golesRaw === 'undefined') return;
      const goles = isNaN(golesRaw) ? 0 : golesRaw;
      if (!partidosPorJugador[j]) partidosPorJugador[j] = {};
      const time = parseFechaToTime(fecha);
      const key = (!isNaN(time)) ? String(time) : String(fecha).trim();
      if (!partidosPorJugador[j][key]) {
        partidosPorJugador[j][key] = {fecha: fecha, goles: goles, __origIdx: idxRow, __time: time};
      } else {
        const cur = partidosPorJugador[j][key];
        // conservar la fila con más goles si hay duplicados en la misma fecha
        if (Number(goles) > Number(cur.goles)) {
          partidosPorJugador[j][key] = {fecha: fecha, goles: goles, __origIdx: Math.min(cur.__origIdx, idxRow), __time: time || cur.__time};
        } else {
          cur.__origIdx = Math.min(cur.__origIdx, idxRow);
        }
      }
  ...
  // Exponer rachas para uso desde la nueva pestaña "Rachas"
  window.rachasList = rachasList || [];
  ...
  // Exponer rachas (derrotas)
  window.rachasListDerrotas = rachasListDerrotas || [];

  // Nueva: calcular rachas de partidos con al menos 1 gol
  const rachasMapGoles = calcularRachasGoles(allRows, 1);
  const rachasListGoles = [];
  Object.entries(rachasMapGoles).forEach(([j, arr]) => {
    (arr||[]).forEach(s => {
      // mostrar rachas de al menos 1 partido (se puede ajustar)
      if ((s.length||s.length===0) && s.length >= 1) {
        rachasListGoles.push({ jugador: j, value: s.length, start: s.start, end: s.end, partidos: (metricasHistoric[j]&&metricasHistoric[j].partidos)||0, __startTime: s.startTime||null });
      }
    });
  });
  rachasListGoles.sort((a,b)=> b.value - a.value || (a.__startTime||0) - (b.__startTime||0));
  window.rachasListGoles = rachasListGoles || [];
  ...
  // Puntos totales en temporada: similar a seasonGoalsMap pero sumando puntos por jugador y año
  const seasonPointsMap = {}; // key: `${year}__${jugador}` -> { jugador, year, points, partidos }
  try {
    // use the already-filtered `rows` so passing a year to renderRecords filters season lists
    rows.forEach((cols, idxRow) => {
      const j = cols[window.idxJugador];
      if (!j) return;
      const fecha = cols[window.idxFecha];
      ...
      if (!year) return;
      const puntosRaw = (typeof window.idxPuntos === 'number' && window.idxPuntos>=0) ? Number(cols[window.idxPuntos]) : NaN;
      const puntos = isNaN(puntosRaw) ? 0 : puntosRaw;
      const key = `${year}__${j}`;
      if (!seasonPointsMap[key]) seasonPointsMap[key] = {jugador: j, year: year, points: 0, partidos: 0};
      seasonPointsMap[key].points += puntos;
      seasonPointsMap[key].partidos += 1;
    });
  } catch(e) { console.warn('Error calculando seasonPointsMap', e); }
  const seasonPointsList = Object.values(seasonPointsMap).map(s=>({jugador: s.jugador, year: s.year, value: s.points, partidos: s.partidos})).sort((a,b)=> b.value - a.value || b.partidos - a.partidos);

  // Partidos ganados y perdidos por temporada
  const seasonWinsMap = {};
  const seasonLossesMap = {};
  try {
    rows.forEach((cols) => {
      const j = cols[window.idxJugador];
      if (!j) return;
      ...
      if (!year) return;
      const pts = (typeof window.idxPuntos === 'number' && window.idxPuntos>=0) ? Number(cols[window.idxPuntos]) : NaN;
      const key = `${year}__${j}`;
      if (!seasonWinsMap[key]) seasonWinsMap[key] = {jugador:j, year: year, wins:0, partidos:0};
      if (!seasonLossesMap[key]) seasonLossesMap[key] = {jugador:j, year: year, losses:0, partidos:0};
      if (!isNaN(pts)) {
        if (pts === 3) seasonWinsMap[key].wins += 1;
        if (pts === 0) seasonLossesMap[key].losses += 1;
      }
      seasonWinsMap[key].partidos += 1;
      seasonLossesMap[key].partidos += 1;
    });
  } catch(e) { console.warn('Error calculando seasonWins/Losses', e); }
  const seasonWinsList = Object.values(seasonWinsMap).map(s=>({jugador: s.jugador, year: s.year, value: s.wins, partidos: s.partidos})).sort((a,b)=> b.value - a.value || b.partidos - a.partidos);
  const seasonLossesList = Object.values(seasonLossesMap).map(s=>({jugador: s.jugador, year: s.year, value: s.losses, partidos: s.partidos})).sort((a,b)=> b.value - a.value || b.partidos - a.partidos);

    // Partidos jugados por temporada (nuevo): calcular por (jugador, año)
    const seasonMatchesMap = {};
    try {
      // usamos `rows` filtradas por año como en los demás season* maps
      rows.forEach((cols) => {
        const j = cols[window.idxJugador];
        if (!j) return;
        const fecha = cols[window.idxFecha];
        ...
        if (!year) return;
        const key = `${year}__${j}`;
        if (!seasonMatchesMap[key]) seasonMatchesMap[key] = {jugador: j, year: year, partidos: 0};
        seasonMatchesMap[key].partidos += 1;
      });
    } catch(e) { console.warn('Error calculando seasonMatchesMap', e); }
    const seasonMatchesList = Object.values(seasonMatchesMap).map(s=>({jugador: s.jugador, year: s.year, value: s.partidos, partidos: s.partidos})).sort((a,b)=> b.value - a.value || b.partidos - a.partidos);

  // Efectividad de Gol (histórico): % de partidos con al menos 1 gol sobre partidos jugados
  const matchesWithGoal = {};
  try {
    rows.forEach(cols => {
      const j = cols[window.idxJugador];
      if (!j) return;
      const g = (typeof window.idxGoles === 'number' && window.idxGoles>=0) ? Number(cols[window.idxGoles]) : 0;
      if (!isNaN(g) && g >= 1) {
        matchesWithGoal[j] = (matchesWithGoal[j] || 0) + 1;
      }
    });
  } catch(e) { console.warn('Error calculando matchesWithGoal', e); }
  const efectividadList = Object.entries(metricasHistoric).map(([j,m])=>{
    const partidos = m.partidos || 0;
    const withGoals = matchesWithGoal[j] || 0;
    const pct = partidos ? (withGoals / partidos * 100) : 0;
    return { jugador: j, value: Number(pct.toFixed(1)), partidos: partidos, withGoals: withGoals };
  }).sort((a,b)=> b.value - a.value || (a.partidos||0) - (b.partidos||0));

  const records = [
    {id:'partidos_jugados', title:'Partidos Jugados', description:'Cantidad de partidos jugados', list: partidosJugados},
    {id:'puntos_totales', title:'Puntos Totales', description:'Suma total de puntos obtenidos por jugador (historico)', list: puntosTotales},
  {id:'racha_victorias', title:'Racha de Victorias', description:'Secuencia de victorias consecutivas', list: rachasList},
  {id:'racha_derrotas', title:'Racha de Derrotas', description:'Secuencia de derrotas consecutivas', list: rachasListDerrotas},
  {id:'racha_goles', title:'Racha Goleadora', description:'Partidos consecutivos con al menos 1 gol', list: rachasListGoles},
  ...
  {id:'puntos_temporada', title:'Puntos Totales en Temporada', description:'Total de puntos obtenidos por jugador en una temporada', list: seasonPointsList},
  {id:'partidos_temporada', title:'Partidos en Temporada', description:'Cantidad de partidos jugados por jugador en una temporada', list: seasonMatchesList},
  // 'perdidos_temporada' removed per request
    {id:'efectividad_gol', title:'Efectividad de Gol', description:'Porcentaje de partidos con al menos 1 gol sobre partidos jugados', list: efectividadList},
  ...
  let recordsToRender = [];
  if (mode === 'seasonOnly') {
    // Para la vista por temporada mostramos los records de temporada disponibles
  recordsToRender = records.filter(r => ['max_goles_temporada','puntos_temporada','partidos_temporada'].includes(r.id));
  } else if (mode === 'rachas') {
    // Vista específica: Rachas (victorias, derrotas y racha goleadora)
    recordsToRender = records.filter(r => ['racha_victorias','racha_derrotas','racha_goles'].includes(r.id));
  } else {
    // Histórico (sin rachas, que ahora viven en su propia pestaña)
  const historicosOrder = ['partidos_jugados', 'puntos_totales', 'goles_total', 'max_goles_partido', 'autogoles'];
  ...
  // Insert a descriptive subheader depending on the active mode
  if (mode === 'seasonOnly') {
    html += `<div class="records-subheader">Lo mejor en una sola temporada: récords y actuaciones legendarias.</div>`;
  } else if (mode === 'rachas') {
    html += `<div class="records-subheader">Rachas históricas: victorias implacables y golpes duros.</div>`;
  } else {
    html += `<div class="records-subheader">Hitos y estadísticas históricas del mejor torneo del mundo.</div>`;
  }
  // Nota: hemos optado por representar la polaridad (bueno/malo) mediante el color de fondo
  recordsToRender.forEach(rec=>{
  // Consider these records as 'bad' (negative metrics)
  const negativeRecords = ['autogoles', 'racha_derrotas'];
  const isBad = negativeRecords.includes(rec.id);
    const rowClass = isBad ? 'record-row record-bad' : 'record-row record-good';
    // Eliminamos badges textuales; el color del contenedor comunica la polaridad
    html += `<div class="${rowClass}" style="display:flex;align-items:center;justify-content:space-between;padding:0.8em;border-radius:8px;">`;
  ...
  // Put the description on the title so hovering the title shows a tooltip
  const safeDesc = (rec.description||'').replace(/"/g,'&quot;');
  html += `<div style="display:flex;align-items:center;gap:0.5em;">`;
  html += `<div class="record-title" data-desc="${safeDesc}" style="width:100%;text-align:center;font-weight:700;color:#ffd700;">${rec.title}</div>`;
  html += `</div>`;
  html += `</div>`;
    // Top 3
  // contenedor de jugadores con clase para estilos responsivos
  html += `<div class="record-players">`;
  // Aplicar filtro de partidos mínimos antes de mostrar top
  // Umbral cambiado a 0 para no excluir jugadores con pocos partidos
  const minPartidosForRecords = 0; // antes: 5
  ...
  if (filtered.length === 0) {
    ...
    for (let gi = 0; gi < groups.length && tilesShown < 3; gi++) {
      ...
      for (let pi = 0; pi < g.items.length && tilesShown < 3; pi++) {
        ...
        anyFromGroup = true;
    const yearBadge = item.year ? `<span class="rec-year">${item.year}</span>` : '';
  const dateRange = (item.start && item.end) ? `<span class="rec-date-range">${item.start} → ${item.end}</span>` : '';
  const singleDate = (item.date) ? `<span class="rec-single-date">${formatDateDDMMYY(item.date)}</span>` : '';
  // extra stats for efectividad_gol: show PJ count and percent suffix
  const extraStats = (rec.id === 'efectividad_gol' && typeof item.partidos !== 'undefined') ? `<span style="color:#9ea5ad;"> · ${item.partidos} PJ</span>` : '';
  // Posición: mostrar número + emoji para las primeras tres posiciones
  const posNum = rank;
  const posEmoji = posNum === 1 ? '🏆' : (posNum === 2 ? '🥈' : (posNum === 3 ? '🥉' : ''));
  const placeBadgeHtml = `<div class="place-badge" title="Puesto ${posNum}"><span class="place-num">${posNum}</span><span class="place-emoji">${posEmoji}</span></div>`;
  // contenedor relativo para permitir badge absolute en top-right
  html += `<div class="player-tile ${placeClass}">`;
  // avatar (badge se colocará absolute en CSS)
  html += `<div class="avatar-wrapper">${avatarFor(item.jugador)}</div>`;
  // Nombre con tooltip (title) para permitir ver nombres largos completos
  html += `<div class="player-info">
    <div class="nombre" title="${escapeHtml(item.jugador)}">${item.jugador}</div>
    <div class="meta"><span class=\"record-value\">${item.value}${rec.id==='efectividad_gol'?'%':''}</span>${extraStats}${yearBadge}${dateRange}${singleDate}</div>
  </div>`;
  // badge absolute top-right
  html += `${placeBadgeHtml.replace('class="place-badge"','class="place-badge absolute"')}`;
  html += `</div>`;
        tilesShown += 1;
      }
      if (anyFromGroup) {
    ...
    html += `</div>`;
  html += `<div style="margin-left:12px;display:flex;align-items:center;gap:0.6em;"> <button class="btn-record-detalle" data-rec="${rec.id}" style="background:#23263a;color:#ffd700;border:1px solid #333;padding:0.4em 0.7em;border-radius:8px;cursor:pointer;">Ver detalle</button></div>`;
    html += `</div>`;
  });
  html += `</div>`;
  ...
  try {
    if (mode === 'seasonOnly') {
      ...
      if (tempCont) tempCont.innerHTML = container.innerHTML;
    } else if (mode === 'rachas') {
      const rachasCont = document.getElementById('recordsRachasContainer');
      if (rachasCont) rachasCont.innerHTML = container.innerHTML;
    } else {
      const histCont = document.getElementById('recordsHistoricos');
      if (histCont) histCont.innerHTML = container.innerHTML;
    }
  } catch (_) {}
  ...
  const minPartidosForRecords = 0;
        ...
        listGroupsToShow.forEach((g, groupIdx) => {
          ...
          const playersHtml = g.items.map(pi => {
            ...
            const extra = (rec.id === 'efectividad_gol') ? `${pi.withGoals || 0} partidos con gol · ${pi.partidos || 0} PJ` : dateHtml;
            const extraHtml = extra ? `<div style="font-size:0.85em;color:#9ea5ad;">${extra}</div>` : '';
            return `<div style="display:flex;align-items:center;gap:0.5em;margin-right:12px;">${avatarFor(pi.jugador)}<div><div style="font-weight:700;color:#fff;">${pi.jugador}</div>${extraHtml}</div></div>`;
          }).join('');
  ...
  try { attachRecordDetailHandlers(document.getElementById('recordsRachasContainer')); } catch(_) {}
...
window.renderRecordsInterno = function(mode, year) {
  ...
  if (mode === 'historico') {
    ...
    // Use core renderer to produce temporada view (so it includes temporada-specific records like goles and puntos)
    try {
      window.renderRecords(year, 'seasonOnly');
      const main = document.getElementById('recordsContainer');
      if (main) temporadaCont.innerHTML = main.innerHTML;
    } catch (e) {
      // Fallback: indicate no datos
      temporadaCont.innerHTML = '<div style="color:#b0b0b0;">No se pudo renderizar récords por temporada.</div>';
    }
  }
};
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const btnRachas = document.getElementById('recordsTabRachasBtn');
  const contHist = document.getElementById('recordsHistoricos');
  const contTemp = document.getElementById('recordsPorTemporada');
  const contRachas = document.getElementById('recordsRachas');
  if (btnHist && btnTemp && contHist && contTemp) {
    btnHist.onclick = function(){ btnHist.classList.add('active'); btnTemp.classList.remove('active'); if(btnRachas) btnRachas.classList.remove('active'); contHist.style.display='block'; contTemp.style.display='none'; if(contRachas) contRachas.style.display='none'; window.renderRecords(); };
    btnTemp.onclick = function(){ btnTemp.classList.add('active'); btnHist.classList.remove('active'); if(btnRachas) btnRachas.classList.remove('active'); contTemp.style.display='block'; contHist.style.display='none'; if(contRachas) contRachas.style.display='none'; window.renderRecords(undefined, 'seasonOnly'); };
    if (btnRachas && contRachas) {
      btnRachas.onclick = function(){ btnRachas.classList.add('active'); btnHist.classList.remove('active'); btnTemp.classList.remove('active'); contRachas.style.display='block'; contHist.style.display='none'; contTemp.style.display='none'; try{ window.renderRecords(undefined, 'rachas'); }catch(e){ console.warn('renderRecords(\'rachas\') missing', e); } };
    }
  }
});
